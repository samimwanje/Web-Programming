...
/**
  ...
  * @param {} txt - The method recieves a string txt which is then printed as Console: "xyz"
  */
 ...
 * @param {} txt - The function recieves the user message as txt.
 */
 ...
 */
function startTimer () {
  startTime = setTimeout(function run () {													// Has the type var because it can be accessed outside the function StartTimeer.
    timePassed++
    quizTime++
    pageTime.innerHTML = timePassed													// Updates the time passed on the page.

    if (quizTime === 10) {																// Check if user spent 10 seconds on the question.
      updateConsole('⏱️Game over, out of time.')
      restartGame()
    }

    if (!restart)																		// Timer do not start a new circle if restart command is called.
    { setTimeout(run, 1000) }
  }, 1000)
}
...
function serverResponse (urlReceive) {
  getResponse(urlReceive).then((data) => {
    ...
    if (JSON.stringify(data.alternatives) !== undefined) {		// Checks if it is a question with many alternatives.
...
function serverPost (urlReceive, body) {
  sendResponse(urlReceive, body).then((data) => {
    updateConsole(data.message)
    answers++																		// Increses the answers answered.
    if (answers === 7) { gameWon(); return } else { serverResponse(data.nextURL) }													// Outputs that the user typed the right answer.
    quizTime = 0
  }).catch((e) => {		// If something is wrong
 ...
 * @returns - false if username is not typed.
 */
function checkUserName () {
  if (userName === '') {																	// If user entered no username.
 ...
 * @returns - False if the url is not an url.
 */
function checkUrl () {
  if (!fetchUrl.includes('http')) {																							// If only url is false.
...
function setUp () {
  if (userName === '') {																									// If username is false or not entered yet.
    userName = userInput.value
    textBox.innerHTML += '\nUsername: ' + userName
    userInput.value = ''

    if (!checkUserName()) {																							// Check if usernamn is false.
      return
    } else {
  ...
  if (!fetchUrl.includes('http')) {																							// If url is false, or not entered yet.
...
userInput.addEventListener('keyup', (event) => {
  if (event.keyCode === 13) {					// Checks if enter key is pressed.
    ...
    if (restart === true && setupComplete == true)									// Clears the console adn the timers if the user restarted the game.
    { clearConsole() }

    // Runs the setup routine until restart is set to false.
    if (restart === true) {
      if (!setupComplete)		// If setup is not complete.
      { setUp() }				// Run setup function.

      if (setupComplete) {				// If the setupcomplete is set to true.
    ...
    if (setupComplete && userInput.value !== '') { // Everytime the user sends a message with ENTER, while the setup is complete.
